...
def generate_peers(rng: np.random.RandomState, n_fit: int, n_eval: int, n_peers: int, peer_complexity: str) -> tuple[np.ndarray, np.ndarray]:
    if peer_complexity == "low":
        Y_p_fit = rng.normal(0.0, 1.0, size=(n_fit, n_peers))
        Y_p_eval = rng.normal(0.0, 1.0, size=(n_eval, n_peers))
    elif peer_complexity == "medium":
        cov = np.full((n_peers, n_peers), 0.85)
        np.fill_diagonal(cov, 1.0)
        Y_p_fit = rng.multivariate_normal(np.zeros(n_peers), cov, size=n_fit)
        Y_p_eval = rng.multivariate_normal(np.zeros(n_peers), cov, size=n_eval)
    elif peer_complexity == "high":
        Y_p_fit = rng.standard_cauchy(size=(n_fit, n_peers))
        Y_p_fit = np.tanh(Y_p_fit) * np.sin(Y_p_fit * 3.14)
        Y_p_eval = rng.standard_cauchy(size=(n_eval, n_peers))
        Y_p_eval = np.tanh(Y_p_eval) * np.sin(Y_p_eval * 3.14)
    else:
        raise ValueError("Invalid peer_complexity")
...
def build_target(rng: np.random.RandomState, Y_p_fit: np.ndarray, Y_p_eval: np.ndarray, complexity: float, noise_std: float) -> tuple[np.ndarray, np.ndarray]:
...
def find_min_budget(y_fit: np.ndarray, X_fit: np.ndarray, y_eval: np.ndarray, X_eval: np.ndarray, lambdas: Iterable[float], tau: float, gamma: float) -> tuple[float, float, float, bool]:
    lambda_list = sorted([float(x) for x in lambdas], reverse=True)
    result = solve_kernel_residuals(
        y_fit=y_fit, Y_p_fit=X_fit, y_eval=y_eval, Y_p_eval=X_eval,
        lambdas=lambda_list, kernel_type={"name": "rbf", "gamma": gamma}
    )
    lambda_star = np.nan
    budget_star = np.nan
    pier_star = np.nan
...
def run_experiment(seed: int, n_targets: int, n_fit: int, n_eval: int, n_peers: int, noise_std: float, tau: float, lambdas: Iterable[float]) -> pd.DataFrame:
    ...
    for p_comp in ["low", "medium", "high"]:
        Y_p_fit, Y_p_eval = generate_peers(rng=rng, n_fit=n_fit, n_eval=n_eval, n_peers=n_peers, peer_complexity=p_comp)
        X_fit, X_eval = standardize_by_fit(Y_p_fit, Y_p_eval)
        gamma = median_heuristic_gamma(X_fit)
        
        for idx, c in enumerate(complexities):
            y_fit, y_eval = build_target(rng=rng, Y_p_fit=Y_p_fit, Y_p_eval=Y_p_eval, complexity=float(c), noise_std=noise_std)
            lambda_star, budget_star, pier_star, reachable = find_min_budget(y_fit=y_fit, X_fit=X_fit, y_eval=y_eval, X_eval=X_eval, lambdas=lambdas, tau=tau, gamma=gamma)
            rows.append({
                "peer_complexity": p_comp,
                "target_id": int(idx),
...
def main():
    ...
    parser.add_argument("--lambdas", type=float, nargs="+", default=[1.0, 3e-1, 1e-1, 3e-2, 1e-2, 3e-3, 1e-3, 3e-4, 1e-4, 3e-5, 1e-5, 3e-6, 1e-6])
    parser.add_argument("--out-csv", type=str, default=str(Path(__file__).resolve().parents[1] / "results/tables/exp2_replacement_complexity_hard.csv"))
    args = parser.parse_args()

    df = run_experiment(
        seed=args.seed, n_targets=args.n_targets, n_fit=args.n_fit, n_eval=args.n_eval,
        n_peers=args.n_peers, noise_std=args.noise_std, tau=args.tau, lambdas=args.lambdas
    )
